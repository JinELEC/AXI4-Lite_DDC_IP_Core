...
The implemented DDC receives a 3 MHz input signal sampled at 12.5 MSPS. A phase accumulator and CORDIC-based NCO generate a 2.5 MHz digital local oscillator, translating the 3 MHz to 500 kHz. The FIR filter then suppresses the unwanted 5.5 MHz mixing component.
...
For a 3 MHz input signal and a 2.5 MHz digital LO, the mixer produces:

**Difference frequency = |3.0 MHz - 2.5 MHz| = 0.5 MHz**

**Sum frequency = 3.0 MHz + 2.5 MHz = 5.5 MHz**

The 500 kHz component is the desired down-converted signal, while the
5.5 MHz component is subsequently suppressed by the FIR filter.
...
- Vivado 2022.1 (synthesis and implementation)
- FPGA: Xilinx Artix-7
